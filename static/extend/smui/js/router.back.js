...
+function($) {
	...
	var Util = {
		...
		getBaseUrlId: function (url) {
			var paramIndex = url.indexOf('/');
			return paramIndex == -1 ? url : url.slice(0,paramIndex);
		}
	};
	...
	Router.prototype._init = function() {
		...
		if (currentUrlObj.fragment) {
			$hashSection = $doc.find('#' + Util.getBaseUrlId(currentUrlObj.fragment));
		}
		if ($hashSection && $hashSection.length) {
			$visibleSection = $hashSection.eq(0);
		} else{
			if(currentUrlObj.fragment == ""){
				currentUrlObj.fragment = "index";
			}
			this.load('#'+currentUrlObj.fragment, true);
		}
		/*
			if ($hashSection && $hashSection.length) {
				$visibleSection = $hashSection.eq(0);
	...
	Router.prototype.load = function(url, ignoreCache) {
		...
		/**
		 * 改为如果指定ID块在当前页,就显示指定块,否则认为加载新页面
		 * @TODO 有一个BUG就是不支持多级目录了.
		 */
		/**
		 * 处理后面带参数的路径
		 */
		var trueUrl = Util.getBaseUrlId(url);
		if($(trueUrl).size() > 0){
			this._switchToSection(Util.getUrlFragment(url));
		}else{
			this._saveDocumentIntoCache($(document), location.href);
			this._switchToDocument(url.replace('#',''), ignoreCache);
		}
	...
	Router.prototype._switchToSection = function(sectionId) {
		...
		var $curPage = this._getCurrentSection(), $newPage = $('#' + sectionId);
	...
	Router.prototype._switchToDocument = function(url, ignoreCache, isPushState, direction) {
		var trueUrl = Util.getBaseUrlId(url),baseUrl = Util.toUrlObject(trueUrl).base;
		if (ignoreCache) {
	...
	Router.prototype._doSwitchDocument = function(url, isPushState, direction) {
		...
		var trueUrl = Util.getBaseUrlId(url);
		var urlObj = Util.toUrlObject(trueUrl);
		var paramIndex = url.indexOf('/');
		var urlParam = paramIndex == -1 ? '' : url.slice(paramIndex);
		 ...
		 * @type {*}
		 */
		$visibleSection.find('.content').append('<div class="dropload-down"><div class="dropload-load"><span class="loading"></span>加载中...</div></div>');
		...
		$visibleSection.data('param',urlParam);
	...
	Router.prototype._loadDocument = function(url, callback) {
		...
		callback = callback || {};
		var self = this,trueUrl=Util.getBaseUrlId(url);

		/**
		 * 使用Sea异步加载代替ajax请求模板
		 */
		App.loadTpl('tpl/'+trueUrl,function (html) {
	...
	Router.prototype._parseDocument = function(url, $doc) {
		var $innerView = $doc.find('.' + routerConfig.sectionGroupClass),trueUrl=Util.getBaseUrlId(url);
		if (!$innerView.length) {
			throw new Error('missing router view mark: ' + routerConfig.sectionGroupClass);
		}

		this._saveDocumentIntoCache($doc, trueUrl);
	};
	...
	Router.prototype._animateDocument = function($from, $to, $visibleSection, direction) {
		...
		var fristId = $from.find('.' + routerConfig.curPageClass).attr('id');
		var animation = direction;

		if(fristId == 'home'){
			animation = 'none';
		}else{
			var fromanimation = $visibleSectionInFrom.attr('animation');
			var toanimation = $to.find('.' + routerConfig.curPageClass).attr('animation');
			if(fromanimation == 'none' && toanimation == 'none'){
				animation = 'none';
			}
		}
	...
	Router.prototype._animateElement = function($from, $to, direction) {
		...
		switch(direction) {
			...
			case DIRECTION.leftToRight:
				...
				break;
			case DIRECTION.opacity:
				classForFrom = 'page-from-block-to-none';
				classForTo = 'page-from-none-to-block';
				break;
			default:
				classForFrom = 'page-from-center-to-left';
				classForTo = 'page-from-right-to-center';
	...
	Router.prototype._back = function(state, fromState) {
		//禁止同页面后退
		if(state.pageId == fromState.pageId){
			return ;
		}
	...
	Router.prototype.reinit = function () {
